*Текущая версия specs актуальна для проектов, которые пишутся отдельно от экосистемы. Экосистема глобально должна включать в себя общие для проектов компоненты и сервисы. При этом, данные specs могут фундаментально меняться от специфики проекта и бизнес-требований.*
...
* **Containers** - собираются из компонентов; могут иметь local context (createLocalContext), могу обращаться к global context.  Внутри containers не должно быть встроенных роутов *e.g. SubscriptionSettings, если находимся внутри page Settings*
...
* Инициализация всех глобальных Services должна происходить в одном файле; в хранилище глобального Context приложения передаются уже инициализированные версии всех необходимых Services. Это делается для обращения к инстансам сервиса из глобального хранилища.
...
* Общее правильно при стилизации - отделяем View от стилей. CSS-in-JS спорно, так как для этого необходимо отказаться от стандартного подхода к стилизации компонентов через файлы стилей, что добавляет сложности системе. При этом, CSS-in-JS при некоторых условиях упрощает theming (ThemeContext OOTB).
